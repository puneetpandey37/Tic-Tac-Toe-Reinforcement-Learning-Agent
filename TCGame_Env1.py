...
class TicTacToe():
    ...
    def is_winning(self, curr_state):
        ...
        Output = False"""
        z_state = np.array(curr_state).reshape((3,3))
        
        row_sum = np.sum(z_state,axis=1)
        col_sum = np.sum(z_state,axis=0)
        
        diagonal_sum_1 = np.sum(np.diagonal(z_state))
        
        if (15 in list(row_sum)) or (15 in list(col_sum)) or (15 == diagonal_sum_1) or (15 == diagonal_sum_1):
            return True
        else:
            return False  
